  ...
  function checkColumns() {
    for (let i = 0; i < 3; i++) {
      ...
      try {testPassed = testTable[0].rows[i].cells.length === 3
        ...
        if (testPassed) {
          console.log(`✅ Row ${i + 1} check passed! 3 td (cells or columns or data) in each row found`);
          columnCheckPass = true;
        } else {
          console.log(`🚫 Row ${i + 1} check failed: 3 td (cells or columns or data) elements needed`);
